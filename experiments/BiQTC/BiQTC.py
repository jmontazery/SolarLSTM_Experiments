...
def prepare_datasets(train, test, validation_split, seq_size, future_target, STEP, batch_size):
    train_stop = int(validation_split*len(train))
    
    x_train_multi, y_train_multi = mutlivariate_data(
        train, train[:, 1], 0, train_stop, seq_size, future_target, STEP)
    
    x_val_multi, y_val_multi = mutlivariate_data(
        train, train[:, 1], train_stop, None, seq_size, future_target, STEP)
    
    x_test_multi, y_test_multi = mutlivariate_data(
        test, test[:, 1], 0 , None, seq_size, future_target, STEP)
...
def create_multistep_model(best_params, model_type, seq_size, future_target, shape):
    ...
    optimizer = tf.keras.optimizers.RMSprop(clipvalue=1.0,learning_rate=learning_rate)
...
file_path = os.path.join(current_dir, 'datasetWithWeather.xlsx')
...
df=dataset[['temp', 'Target', 'month', 'season', 'year']]
...
with open('lstm_params.json', 'r') as file:
...
index = 1
feature_index = 1  # Specify which feature/column to plot
...
model_types=['bidirectional_lstm'] #, 'stacked_lstm', 'bidirectional_lstm'
...
for model_type in model_types:
    ...
    for train_index, test_index in splits.split(dataset):
        ...
        train, test, validation_split, seq_size, future_target, STEP, batch_size)
